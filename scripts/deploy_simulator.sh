#!/bin/bash
#
# Deploy Solax PV Simulator to Home Assistant
#
# Usage: ./scripts/deploy_simulator.sh [OPTIONS]
#
# Options:
#   -h, --host HOST     Remote host (or set DEPLOY_HOST/SOLAX_SIMULATOR_HOST in .env)
#   -p, --path PATH     Remote config path (or set DEPLOY_PATH/SOLAX_SIMULATOR_PATH in .env)
...
REMOTE_HOST="${SOLAX_SIMULATOR_HOST:-${DEPLOY_HOST:-}}"
REMOTE_PATH="${SOLAX_SIMULATOR_PATH:-${DEPLOY_PATH:-}}"
RESTART=false
DRY_RUN=false
...
SOURCE_DIR="$PROJECT_ROOT/custom_components/solax_pv_simulator"
...
DEST_DIR="$REMOTE_PATH/custom_components/solax_pv_simulator"

echo -e "${BLUE}Solax PV Simulator Deployment${NC}"
echo "================================"
...
echo "1. Go to Settings → Devices & Services → Add Integration"
echo "2. Search for 'Solax PV Simulator'"
echo "3. Configure battery capacity, PV power, etc."
echo "4. Use the simulated entities with Solar Mind integration"
